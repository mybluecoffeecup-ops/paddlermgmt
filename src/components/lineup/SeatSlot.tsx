...
export function SeatSlot({
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "flex min-h-[64px] flex-col justify-center rounded-lg border-2 border-dashed p-1 transition-colors",
        seat.isCrewRole
        ...
        isOver && "border-teal-400 bg-teal-50 dark:bg-teal-500/10"
      )}
    >
      {occupant ? (
        <PaddlerChip paddler={occupant} compact mismatch={mismatch} />
      ) : (
        <div className="flex flex-col items-center justify-center gap-0.5 py-1.5 text-slate-300 dark:text-white/20">
