...
`src/hooks/app-data.tsx` (`AppDataProvider`/`useAppData`) is the single source of truth for all domain data (profiles, sessions, attendance, races, race_commitments, lineups) and is mounted in `src/app/(app)/layout.tsx`, wrapping every route in that route group (`/`, `/sessions`, `/races`, `/lineups/*`, `/profile`, `/info`, `/orders`). The root `src/app/layout.tsx` only sets up fonts and the `<html>`/`<body>` shell — it does *not* mount the provider, so `/login` and `/signup` (outside the `(app)` group) have no access to `useAppData()` by design.
...
- `/sessions` is fully coach-only too, but `src/app/(app)/sessions/[sessionId]/page.tsx` (the session detail view) is **not** — paddlers can view/comment on it. So `RequireCoach` is wrapped **inline inside `src/app/(app)/sessions/page.tsx`'s own return**, not in a `sessions/layout.tsx` — a layout there would incorrectly cascade the gate down to `/sessions/[sessionId]` too. The same reasoning applies to `/races` vs. `/races/[raceId]`.
- `/races` is a hybrid: the page itself is open to both roles (paddlers need to see it for race signups), but the coach-only `RaceManager` panel within it is conditionally rendered with an inline `resolved && role === "coach"` check (mirroring `RequireCoach`'s internal logic, see the race condition note above) rather than being wrapped in `RequireCoach` itself — `RequireCoach` redirects non-coaches away entirely, which is wrong here since paddlers should still land on `/races`.
...
`SessionManager.tsx` (rendered on the Session Mgmt page, `/sessions`; its component directory is still named `src/components/command-center/` for historical reasons — that directory is shared by both `/sessions` and `/races`, it isn't a leftover) owns both browsing sessions and creating them: a "+ New Session" button in its `CardHeader` toggles an inline `NewSessionForm` that *replaces* the session-picker list while open (no modal component exists anywhere in this codebase — every "create" flow is an inline `Card` form, not a popup). `type` is a closed `SessionType` union (`"Training" | "Race Simulation" | "Race"`, `SESSION_TYPE_OPTIONS` in `src/types/index.ts`) backed by the `session_type` Postgres enum (`0004_session_type_enum.sql`) — unlike most other fields, this one used to be a free-text column and was tightened after the fact, so don't assume every string is a valid session type without checking this union first.
...
- `/races` — Race Mgmt: coach-only race management (`RaceManager`, conditionally rendered inline for `role === "coach"`) plus a work-in-progress "Race Signups" placeholder, open to both roles
