...
- **Info** (`/info`) — a Socials panel (Instagram/Facebook/TikTok links) and a Sponsors panel side by side: sponsor tiles show a logo (real image where available, monogram fallback otherwise) plus a concise discount blurb, and clicking one opens a slide-in detail drawer with the full redemption terms and a link to the sponsor's site. Below that, a work-in-progress placeholder for the team info/reference document library
