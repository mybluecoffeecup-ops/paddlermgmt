...
- **Lineup builder** (`/lineups`, `/lineups/[lineupId]`; standalone "Lineup Tool" tab in `prototype.html`) — drag-and-drop seat assignment across boat layouts (DB12, DB22, V6), including dedicated Drummer/Steer seats on dragon boats (centered between the Left/Right columns), a color key for the side-preference/mismatch indicators and Pacer/Steer/Drummer role icons, and a live left/right (or odd/even, for the single-file V6) weight-balance telemetry bar. Click any bench paddler to auto-assign them to their best-fit open seat, drag-and-drop for manual control, or hit **Auto-fill** to bulk-seat every remaining bench paddler at once (crew roles first, then greedily balanced by weight across the same left/right or odd/even groups the telemetry bar tracks). Edits stay local until you hit **Save** — a **Discard** option reverts them, and trying to navigate away with unsaved changes opens a Save/Discard/Cancel confirmation dialog. Lineups can be built against a session (bench sourced from session attendance) or a race (bench sourced from race commitments) via a Session/Race toggle on lineup creation, and a lineup's boat type (DB12/DB22/V6) can be switched after creation from inside the editor. Restricted to coaches client-side
- **Info** (`/info`) — a Socials panel (icon-only Instagram/Facebook/TikTok links, opening each platform in a new tab) and a Sponsors panel side by side: sponsor tiles show a logo (real image where available, monogram fallback otherwise), name, and a concise discount blurb stacked vertically (fitting all sponsors on one row on wide screens), and clicking one opens a slide-in detail drawer with the full redemption terms and a link to the sponsor's site. Below that, a **Team Info** panel — a coach-managed library of links to web documents (Google Docs, Slides, Forms, Sheets, or any other URL); clicking one opens a near-fullscreen in-app viewer that embeds Google documents directly via iframe (other links open in a new tab instead), and coaches get inline add/edit/delete controls for the list
- **Orders / Shop** (`/orders`) — team apparel ordering, no in-app payment. A "Shop" tab lets paddlers browse active styles (image, name, description), pick an in-stock size (out-of-stock sizes shown disabled) with an optional shared size-chart lookup, and build a cart that persists per-user across a refresh (localStorage, not a server draft). "Submit order" creates a `pending` order with no stock impact yet; a "My Orders" tab shows a paddler's own order history with status badges, and lets them edit or cancel a still-pending order. Coaches get a third "Admin Tools" tab, laid out by workflow priority: a status-filterable Order Queue and a Current Stock snapshot (per-style totals left, placeholder Restock button) side by side up top, then Styles & Stock and Size Charts CRUD side by side further down, and a Sales History rollup of accepted orders last. Size charts support a real image upload (read client-side to a data URL, no server storage) with a preview thumbnail before saving, and clicking any chart in the list expands an inline image preview. Each panel's list scrolls internally after ~4 rows instead of growing the page. Accepting an order is the only path that decrements stock, done atomically via a Postgres RPC (`accept_shop_order`) so a double-click or concurrent accept can't over-decrement — a coach sees an inline error if stock has since run out. Order submission fires a coach-only in-app notification (the shared notifications table's `audience` column keeps it out of paddlers' notification bells). Seeded with the club's real Hazonas apparel lineup (Drinking Shirt, Rashie/Hooded Rashie in standard and slimfit, Singlet, T-Shirt) — real product photos and size charts under `public/shop/`, browse thumbnails cropped to the design side of the artwork, full uncropped image in the detail popup
- **Auth** — real Supabase Auth (email/password) once a Supabase project is configured: `/login` and `/signup`, route protection via `src/proxy.ts`, and a `profiles` row auto-created per signup. Paddler vs. Coach view is derived from `profiles.is_coach`; Sessions and Lineups are fully coach-gated client-side via `RequireCoach`, while Calendar and Orders are open to both roles with their coach-only management panels (race/event forms, the Shop's "Admin Tools" tab) gated inline instead, since a coach is also a paddler — `/`, `/profile`, `/calendar`, `/info`, and `/orders` stay open to both roles. Without Supabase configured, the app falls back to a Paddler/Coach UI toggle over mock data (no login required) for local development
...
src/
  ...
  components/
    ...
    info/                  # SocialsCard, SponsorsCard + SponsorDrawer (slide-in detail panel), sponsors-data.ts,
                           # TeamInfoCard + TeamDocumentViewer (Team Info document library + embed viewer)
    shop/                  # ShopBrowseGrid, ShopStyleDetail (size chart + add to cart), ShopCart, MyOrders,
                           # ShopStyleManager, ShopSizeChartManager, ShopOrderQueue, ShopCurrentStock,
                           # ShopSalesHistory (coach, "Admin Tools" tab)
    nav/                  # AppShell (role badge/sign-out or demo toggle, navigation), BrandMark, NotificationBell
    ui/                   # shared primitives, incl. PaddlesIcon, WorkInProgress, Dialog (Save/Discard/Cancel
                           # confirmations), and calendar category badges
  hooks/
    app-data.tsx           # single data-fetching context consumed by all routes
    use-shop-cart.ts        # client-only cart, persisted to localStorage per user
    use-unsaved-changes-guard.ts # blocks in-app nav/tab-close while the lineup editor has unsaved changes
  lib/
    api/                  # Supabase CRUD calls per table (incl. comments, notifications, workout_program,
                           # team_documents, shop styles/sizes/orders — the latter's accept flow calls the
                           # accept_shop_order() Postgres RPC instead of a plain update)
    ...
    auto-lineup.ts         # bulk Auto-fill: crew roles first, then weight-balanced side assignment
    ...
    document-utils.ts     # Google Docs/Slides/Forms/Sheets URL -> embeddable iframe URL detection
...
public/sponsors/          # sponsor logo images referenced by SponsorsCard/SponsorDrawer
public/shop/               # product photos + size-charts/ referenced by MOCK_SHOP_STYLES/MOCK_SHOP_SIZE_CHARTS
