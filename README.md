...
- **Race Mgmt** (`/races`) — a race management panel for coaches to add races to the calendar (date, location, discipline, competitiveness level, and Gender/Age Range/Other category tags), which immediately appear on paddlers' dashboards, with each race row tapping through to its detail page. A unified Race Signups section (Going/Maybe/Not Going for the whole crew, in one place) is a work-in-progress placeholder for now — RSVPing still works today from each race's own detail page and from the Home dashboard's Race Tracking card
...
- **Auth** — real Supabase Auth (email/password) once a Supabase project is configured: `/login` and `/signup`, route protection via `src/proxy.ts`, and a `profiles` row auto-created per signup. Paddler vs. Coach view is derived from `profiles.is_coach`; Session Mgmt and Lineups are fully coach-gated client-side via `RequireCoach`, while Race Mgmt is open to both roles (coach-only race management, crew-wide race signups) since a coach is also a paddler — `/`, `/profile`, `/races`, `/info`, and `/orders` stay open to both roles. Without Supabase configured, the app falls back to a Paddler/Coach UI toggle over mock data (no login required) for local development
...
src/
  app/
    (app)/                # routes wrapped in AppDataProvider + AppShell: /, /profile, /sessions, /races,
  ...
  components/
    ...
    command-center/        # roster, filters, session manager, race manager, WorkoutProgramPanel
                           # (shared by the /sessions and /races pages)
    ...
    ui/                   # shared primitives, incl. PaddlesIcon and WorkInProgress
